...
module MakeInterval (Endpoint : ORDERED_TYPE) =
  struct
    ...
    let create (low : Endpoint.t) (high : Endpoint.t) : interval =
      if compare low high = 1 then Empty
    ...
    let contains (intvl : interval) (x : Endpoint.t) : bool =
      match intvl with
      | Empty -> false
      | Interval (low, high) -> compare low x <= 0 && compare x high <= 0
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      ...
      | Interval (low1, high1), Interval (low2, high2) ->
          let low = if compare low1 low2 <= 0 then low2 else low1 in
          let high = if compare high1 high2 <=0 then high1 else high2 in
          create low high
  end ;;
...
let intvl2 = IntInterval.create 5 7 ;;
